...
const AccountType = ({ showAccountType, handleChange, toggleAccountType }) => {
  const SPACING = config.hp("2%");
  const ITEM_SIZE = SPACING * 4.9;
  const frequencyOptions = ["Checking", "Savings", "Other"];
    ...
    <>
      {showAccountType ? (
        <View style={styles.modalContainer}>
          ...
          />
          <TouchableOpacity onPress={toggleAccountType}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View>
      ) : undefined}
