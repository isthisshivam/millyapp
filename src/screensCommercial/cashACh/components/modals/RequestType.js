...
const RequestType = ({ showRequestType, handleChange, toggleRequestType }) => {
  const SPACING = config.hp("2%");
  const ITEM_SIZE = SPACING * 4.9;
  const frequencyOptions = ["PPD", "CCD"];
    ...
    <>
      {showRequestType ? (
        <View style={styles.modalContainer}>
          ...
          />
          <TouchableOpacity onPress={toggleRequestType}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View>
      ) : undefined}
